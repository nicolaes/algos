...
def bfs_a_star(init_floor: int, init_pos: int, init_right: bool):
    ...
    while q:
        ...
        just_above_el = crt.pos if crt.pos in exits[next_floor] else None
        next_neighbors = neighbor_el_pos(next_floor, crt.pos)
        for next_pos in just_above_el, *next_neighbors:
        ...
        if next_floor in forced_elevators: # forced elevator
            ...
            if crt.spare_elevators > 0 or not elevator_found:
                use_spare = elevator_found # use spare if not on an empty floor
                if crt.floor < nb_floors - 2:
                    ...
                    for pos in two_levels_above:
                        if pos > l_limit and pos < r_limit and pos != crt.pos:
                            q.put(new_elevator(crt, pos - 1, use_spare))
                            # q.put(new_elevator(crt, pos, use_spare))
                            q.put(new_elevator(crt, pos + 1, use_spare))

                q.put(new_elevator(crt, crt.pos, use_spare))     
